import { ColorToGrTextureFormat_t, GrTextureFormat_t, hw_aspect_table, lodToLod, wh_aspect_table, _grMipMapHostSize, _grMipMapHostWH, _gr_aspect_index_table, } from "./glide.js";
const assert = (a) => {
    if (!a) {
...
function getPreambleEndIndex(ui8Array) {
    const lastPreambleIndex = ui8Array.findIndex((code) => !ALLOWED_ASCII.includes(code));
    return lastPreambleIndex;
}
function parsePreamble(preambleSlice, name) {
    const preambleText = preambleSlice.reduce((acc, code) => acc + String.fromCodePoint(code), "");
    const preamble = preambleText
        ...
        switch (index) {
            ...
            case 1: {
                ...
                assert(ColorToGrTextureFormat_t[color]);
                acc["COLOR"] = ColorToGrTextureFormat_t[color];
                return acc;
            }
            case 2: {
    ...
    preamble.lod_range.small =
        lodToLod[preamble.lod_range.small];
    preamble.lod_range.large =
        lodToLod[preamble.lod_range.large];
    {
        let mem_required = 0;
        for (let lod = preamble.lod_range.large; lod <= preamble.lod_range.small; ++lod) {
            mem_required +=
                _grMipMapHostSize[_gr_aspect_index_table[preamble.aspect_ratio.aspect_ratio]][lod] << Number(preamble.COLOR >= GrTextureFormat_t.GR_TEXFMT_16BIT);
        }
        preamble.mem_required = mem_required;
    }
    return preamble;
}
function parseImage(preamble, imageSlice, name) {
    switch (preamble.COLOR) {
        ...
        default:
            console.error(new Error(`${GrTextureFormat_t[preamble.COLOR]} is not supported in file ${name}!`));
    }
}
async function printFile(name) {
    ...
    for (let lod = preamble.lod_range.large; lod <= preamble.lod_range.small; ++lod) {
        ...
        const container = document.createElement("div");
        {
            ...
            const { COLOR, width, height, aspect_ratio: { aspect_ratio }, } = preamble;
