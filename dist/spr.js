...
function getChars(ui8Array, start, end) {
    let result = "";
...
function getInt(ui8Array, start) {
    return ((ui8Array[start + 3] << 24) |
        (ui8Array[start + 2] << 16) |
        (ui8Array[start + 1] << 8) |
        ui8Array[start]);
}
function getShortInt(ui8Array, start) {
    return (ui8Array[start + 1] << 8) | ui8Array[start];
}
function getNthPage(ui8Array, page) {
    const Disp = getInt(ui8Array, 8 * page + 12);
    console.log(getInt(ui8Array, 8 * page + 12), getShortInt(ui8Array, 8 * page + 12));
...
export function getDataForSpr(ui8Array, name) {
    ...
    const result = new Array(SprHeader.Count);
